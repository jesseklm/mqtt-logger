...
class MqttHandler:
    def __init__(self, config: dict, sub_topics=None, message_callback=None) -> None:
        self.topic_prefix: str = config.get('mqtt_topic', 'mqtt_logger/').rstrip('/') + '/'
        ...
        client_id = hashlib.md5(f'mqtt_logger-{self.host}{self.port}{self.topic_prefix}'.encode()).hexdigest()
